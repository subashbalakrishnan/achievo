...
$query= 'SELECT concat(firstname," ",lastname) as name, GROUP_CONCAT(hours.remark SEPARATOR "\n") as remarks,  GROUP_CONCAT(round(hours.time/60,1) SEPARATOR "\n") as time  FROM hours join person where person.id=hours.userid and activitydate between "'.$startdate.'" and "'.$enddate.'" GROUP BY person.userid';

//echo $query;


$link = mysql_connect('localhost', 'iqss_iqssdotin', 'iq$$!n2012');
if (!$link) {
...
$sheet1[]=array("Name","Description","Hours");

if ($row = mysql_fetch_array($result)) {

do {

	$sheet1[]=array($row["name"],$row["remarks"],$row["time"]);
...
foreach($arr as $wbname=>$rows)
{
    ...
    $worksheet->setColumn(1,1,100.00);
